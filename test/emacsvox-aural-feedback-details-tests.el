...
(ert-deftest emacsvox-aural-feedback-details-indentation-links-and-replays-its-voice ()
  "A real indented line exposes its spoken label without changing the recording."
  (require 'emacsvox-speak)
  ...
  (emacsvox-test--with-home-context
   (let* ((record
               ...
               (let ((position 0))
                 (while (< position (length prepared))
                   (let ((end (next-single-property-change
                               position emacsvox-aural-concrete-plan-property prepared (length prepared))))
                     (push (emacsvox-aural--freeze-presentation-plan
                            (emacsvox-aural-concrete-plan-at position prepared)
                            (substring prepared position end)) plans)
                     (setq position end))))
               (emacsvox-aural--make-history-record (nreverse plans) nil 423))))
          (before (emacsvox-aural--history-value record))
          (report (emacsvox-aural-feedback-details record))
          (emacsvox-aural-presentation-history (list record))
          played opened spoken)
     ...
     (should (search-forward "Before speech: say \"indent 2\" using indent" nil t))
     (beginning-of-line)
     (let ((position (point)))
       (forward-line -1)
       (cl-letf (((symbol-function 'emacsvox-aural-ui--speak-control)
                  (lambda (text) (setq spoken text))))
         (emacsvox-aural-feedback-details-next-heading))
       (should (= (point) position))
       (should-not (string-match-p "Test" spoken)))
     (cl-letf (((symbol-function 'emacsvox-aural-preview-play-runs)
                (lambda (runs &rest _)
                  (should emacsvox-aural--history-recording-inhibited)
                  (setq played runs))))
       (call-interactively (key-binding (kbd "O"))))
     (let* ((plan (caar played))
            (actions (emacsvox-aural-concrete-plan-before plan)))
       (should (= (length played) 1))
       (should (= (length actions) 1))
       (should (equal (emacsvox-aural-concrete-action-text (car actions)) "indent 2"))
       (should (eq (emacsvox-aural-concrete-action-voice-request (car actions)) 'indent))
     ...
     (search-forward "Edit named voice indent")
     (backward-char)
     (cl-letf (((symbol-function 'emacsvox-aural-voice-editor-open)
                (lambda (&rest args) (setq opened args))))
       (call-interactively (key-binding (kbd "RET"))))
     (should (eq (nth 1 opened) 'indent))
     (should (eq (nth 2 opened) report))
     (should (equal (nth 3 opened) "indent 2"))
     (should-not emacsvox-aural-feedback-details--drafts)
     (should (equal before (emacsvox-aural--history-value record)))
     (should (equal emacsvox-aural-presentation-history (list record))))))

(ert-deftest emacsvox-aural-feedback-details-spoken-labels-keep-phase-and-settings ()
  "Each label replays alone, including After labels and voices with context tuning."
  (emacsvox-test--with-feedback-report
   (let* ((indices (emacsvox-test--feedback-field "Subject"))
          (plan (emacsvox-aural-feedback-details--plan (car indices)))
          (cue (emacsvox-aural--make-concrete-action :kind 'cue :cue 'button))
          (first (emacsvox-aural--make-concrete-action
                  :kind 'speech :text "First label" :voice-request 'indent
                  :voice-style '(:average-pitch 2) :balance -0.5))
          (second (emacsvox-aural--make-concrete-action
                   :kind 'speech :text "Last label"
                   :voice-request '(:preset indent :average-pitch 7)
                   :voice-style '(:average-pitch 7) :balance 0.5))
          played)
     (setf (emacsvox-aural-concrete-plan-before plan) (list cue first)
           (emacsvox-aural-concrete-plan-after plan) (list second cue))
     (emacsvox-aural-feedback-details-toggle)
     (let ((snapshot (emacsvox-aural--history-value record)))
       (dolist (case (list (list "Before" first) (list "After" second)))
         (goto-char (point-min))
         (search-forward (concat (car case) " speech:"))
         (beginning-of-line)
         (cl-letf (((symbol-function 'emacsvox-aural-preview-play-runs)
                    (lambda (runs &rest _) (setq played runs))))
           (call-interactively (key-binding (kbd "RET"))))
         (let* ((replay (caar played))
                (actions (append (emacsvox-aural-concrete-plan-before replay)
                                 (emacsvox-aural-concrete-plan-after replay)))
                (timeline (car (emacsvox-aural--build-structured-timeline 2 41 played)))
                (spans (plist-get timeline :spans)))
           (should (equal actions (list (cadr case))))
           (should (= (length spans) 1))
           (should (equal (plist-get (aref spans 0) :text)
                          (emacsvox-aural-concrete-action-text (cadr case))))
           (should (equal (plist-get (aref spans 0) :logical_voice_id) "indent")))
         (should-error (call-interactively (key-binding (kbd "R"))) :type 'user-error))
       (should (equal snapshot (emacsvox-aural--history-value record)))))))

(ert-deftest emacsvox-aural-feedback-details-unnamed-and-truncated-labels-are-guarded ()
  "Unowned voices stay playable, while incomplete recordings cannot replay."
  (emacsvox-test--with-feedback-report
   (let* ((indices (emacsvox-test--feedback-field "Subject"))
          (plan (emacsvox-aural-feedback-details--plan (car indices)))
          (action (emacsvox-aural--make-concrete-action
                   :kind 'speech :text "Raw label" :voice-request '(:average-pitch 4))))
     (setf (emacsvox-aural-concrete-plan-after plan) (list action))
     (emacsvox-aural-feedback-details-toggle)
     (search-forward "After speech:")
     (beginning-of-line)
     (let ((position (point)))
       (should (search-forward "No editable named palette voice" nil t))
       (goto-char position)
       (dolist (key '("C" "r"))
         (should-error (call-interactively (key-binding (kbd key))) :type 'user-error))
       (should-not emacsvox-aural-feedback-details--drafts)
       (cl-letf (((symbol-function 'emacsvox-aural-presentation-record-effective-payload-truncated-p)
                  (lambda (_) t))
                 ((symbol-function 'emacsvox-aural-preview-play-runs)
                  (lambda (&rest _) (ert-fail "Played incomplete recording"))))
         (should-error (call-interactively (key-binding (kbd "O"))) :type 'user-error))))))
