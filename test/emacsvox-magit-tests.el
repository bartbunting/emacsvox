...
(ert-deftest emacsvox-magit-confirmation-prompt-uses-main-speech ()
  "Both short and long file confirmations speak on the main stream."
  (dolist (slow '(nil t))
    (let* ((magit-no-confirm nil)
           (magit-slow-confirm slow)
           ...
           speech notifications opening-speech opening-notifications result)
      (cl-letf (((symbol-function 'emacsvox-icon) #'ignore)
                ...
                ((symbol-function 'emacsvox-test--confirm-file)
                 (lambda ()
                   (interactive)
                   (setq result
                         (magit-confirm-files 'trash '("example.txt")
                                              nil nil t)))))
        (add-hook 'minibuffer-setup-hook
                  (lambda ()
                    (setq opening-speech (copy-sequence speech)
                          opening-notifications (copy-sequence notifications)))
                  t)
        (define-key global-map (kbd "C-c q") #'emacsvox-test--confirm-file)
        (save-window-excursion
          (let ((noninteractive nil)
                (use-dialog-box nil)
                (y-or-n-p-use-read-key nil))
            (execute-kbd-macro
             (vconcat (kbd "C-c q") (kbd (if slow "n o RET" "n")))))))
      (should-not result)
      (should-not opening-notifications)
      (should (= 1 (length opening-speech)))
      (should (string-match-p "Trash.*example.txt.*(" (car opening-speech))))))
