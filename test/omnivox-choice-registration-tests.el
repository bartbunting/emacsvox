...
(ert-deftest omnivox-choice-registration-captures-raw-provenance-before-acknowledgement ()
  (omnivox-test--with-choice-registration
   (setq tts-notify-process nil)
   (let* ((owned (tts--dispatch-copy-data (emacsvox-aural-voice-runtime--owned "bolden")))
          (before (tts--dispatch-copy-data owned)))
     (cl-letf (((symbol-function 'emacsvox-aural-voice-runtime--owned)
                (lambda (id &rest _)
                  (when (member id '("bolden" "voice-bolden")) owned))))
       (omnivox-apply-voice-configuration))
     (let ((request (cdr (car requests))))
       (should-not (plist-member request :choice-provenance))
       (should-not (plist-member request :choice-tuning-unapplied)))
     (aset (plist-get (car (plist-get owned :choices)) :id) 0 ?z)
     (setf (plist-get owned :palette) 'changed)
     (setf (plist-get owned :definition) '(:richness 9))
     (omnivox--dispatch-control-response speaker (omnivox-test--choice-registration-ack (car requests)))
     (let* ((snapshot (process-get speaker omnivox--choice-registration-property))
            (provenance (plist-get (plist-get snapshot :content) :choice-provenance)))
       (should (= (length provenance) 2))
       (dolist (row (append provenance nil))
         (should (eq (plist-get row :palette) 'reading))
         (should (eq (plist-get row :name) 'bolden))
         (should (equal (plist-get row :names) (plist-get before :names)))
         (should (equal (plist-get row :choices) (plist-get before :choices)))
         (should (equal (plist-get row :definition) (plist-get before :definition)))
         (should (equal (plist-get row :shared)
                        (emacsvox-aural-voice-runtime--definition-style (plist-get before :definition)))))))))

(ert-deftest omnivox-choice-registration-owner-change-advances-identical-wire-content ()
  (omnivox-test--with-choice-registration
   (setq tts-notify-process nil)
   (let ((resolve (symbol-function 'emacsvox-aural-voice-runtime--owned))
         (palette 'reading))
     (cl-letf (((symbol-function 'emacsvox-aural-voice-runtime--owned)
                (lambda (id &rest args)
                  (when-let* ((owned (apply resolve id args)))
                    (plist-put owned :palette palette)))))
       (omnivox-register-logical-voices)
       (let ((first (car requests)))
         (setq palette 'other)
         (omnivox-register-logical-voices)
         (let* ((second (car requests))
                (first-wire (copy-tree (cdr first)))
                (second-wire (copy-tree (cdr second))))
           (should (< (plist-get first-wire :registry_generation)
                      (plist-get second-wire :registry_generation)))
           (dolist (key '(:request_id :registry_generation))
             (cl-remf first-wire key) (cl-remf second-wire key))
           (should (equal first-wire second-wire))
           (omnivox--dispatch-control-response speaker (omnivox-test--choice-registration-ack second))
           (omnivox--dispatch-control-response speaker (omnivox-test--choice-registration-ack first))
           (should (eq (plist-get (aref (plist-get (plist-get
                                                     (process-get speaker omnivox--choice-registration-property)
                                                     :content) :choice-provenance) 0) :palette)
                       'other))))))))

(ert-deftest omnivox-choice-registration-bounds-metadata-before-sending-policy-or-registry ()
  (omnivox-test--with-choice-registration
   (let ((tts--dispatch-metadata-limit 1))
     (should-error (omnivox-apply-voice-configuration))
     (should-not requests))))

(ert-deftest omnivox-choice-registration-freezes-hash-members-and-mutable-wire-inputs ()
  (let* ((string (copy-sequence "original"))
         (patch (make-hash-table :test #'equal))
         (content (list :definitions (vector (list :id string :patch patch))))
         (copy (tts--dispatch-copy-data content)))
    (puthash "richness" (list :value string) patch)
    (let ((second (tts--dispatch-copy-data content)))
      (aset string 0 ?X)
      (clrhash patch)
      (should (equal (plist-get (aref (plist-get second :definitions) 0) :id) "original"))
      (should (equal (plist-get (gethash "richness" (plist-get (aref (plist-get second :definitions) 0) :patch)) :value)
                     "original")))
