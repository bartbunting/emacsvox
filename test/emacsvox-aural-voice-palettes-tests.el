...
(ert-deftest emacsvox-aural-voice-palettes-tools-source-and-known-uses ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-owned-runtime
    (let ((emacsvox-aural-user-rules '((:id mapped :render (:content (:voice voice-bolden)))))
          (voice-setup-face-voice-table (make-hash-table :test #'eq)))
      (puthash 'fixture-face '(voice-bolden voice-smoothen) voice-setup-face-voice-table)
      (with-temp-buffer
        (emacsvox-aural-voice-palette-previews-mode)
        (setq emacsvox-aural-voice-palette-previews-palette 'source-child)
        (emacsvox-aural-voice-palette-previews-refresh 'bolden)
        (should (equal (aref (cadr (assq 'bolden tabulated-list-entries)) 5) "source-base"))
        (should (string-match-p "Source source-base" (emacsvox-aural-voice-palette-previews--row-summary))))
      (let ((uses (emacsvox-aural-voice-palettes--known-uses 'bolden)))
        (should (member "Personal rules" uses))
        (should (member "Global: face fixture-face" uses)))
      (should-not (emacsvox-aural-voice-palettes--voice-reference-p '(:personality voice-bolden) 'voice-bolden)))))

(ert-deftest emacsvox-aural-voice-palettes-tools-startup-preserves-saved-and-live-fields ()
  :tags '(voice-palette-tools)
  ...
  (emacsvox-test--with-voice-save
    (let ((emacsvox-aural-profile-registry (make-hash-table :test #'eq))
      ...
      (emacsvox-aural-register-profile
       '(:id fixture-startup :summary "Saved setup" :feature-fragments nil :sound-pack nil
         :voice-palette reading-owned :spatial (:enabled nil :output mono)))
      (emacsvox-aural-save-user-data)
      (let* ((before (emacsvox-aural-read-user-data))
             (expected (copy-tree before))
             (current emacsvox-aural-voice-palette-override)
             (emacsvox-aural-spatial-enabled t))
        (setf (plist-get (car (plist-get expected :profiles)) :voice-palette) 'alternative-owned)
        (cl-letf (((symbol-function 'tts-apply-voice-configuration) (lambda (&rest _) (ert-fail "Startup applied live settings"))))
          (should (eq (emacsvox-aural-profile-set-startup-palette 'alternative-owned) 'fixture-startup)))
        (should (equal expected (emacsvox-aural-read-user-data)))
        (should (eq emacsvox-aural-voice-palette-override current))
        (should emacsvox-aural-spatial-enabled)
        (should (equal (plist-get (emacsvox-aural-profile-entry-data (emacsvox-aural-profile-entry 'fixture-startup)) :spatial)
                       '(:enabled nil :output mono)))
        (cl-letf (((symbol-function 'emacsvox-aural--write-user-data) (lambda (&rest _) (error "Write failure"))))
          (should-error (emacsvox-aural-profile-set-startup-palette 'reading-owned)))
        (should (equal expected (emacsvox-aural-read-user-data)))
...
(ert-deftest emacsvox-aural-voice-palettes-tools-new-startup-is-reviewed-and-conflict-checked ()
  ...
  (emacsvox-test--with-voice-save
    (let* ((emacsvox-aural-profile-registry (make-hash-table :test #'eq))
           (emacsvox-aural-active-profile nil)
           (profile (emacsvox-aural-capture-profile-data 'new-setup "Reviewed setup"))
           (current emacsvox-aural-voice-palette-override))
      (let ((emacsvox-aural-spatial-enabled (not emacsvox-aural-spatial-enabled)))
        (should-error (emacsvox-aural-profile-set-startup-palette 'alternative-owned profile)))
      (should-not (emacsvox-aural-profile-entry 'new-setup))
      (emacsvox-aural-profile-set-startup-palette 'alternative-owned profile)
      (let ((saved (emacsvox-aural-read-user-data)))
        (should (eq (plist-get saved :active-profile) 'new-setup))
        (should (eq (plist-get (car (plist-get saved :profiles)) :voice-palette) 'alternative-owned))
        (should (eq current emacsvox-aural-voice-palette-override))
        (let ((changed (copy-tree saved)))
          (setf (plist-get (car (plist-get changed :profiles)) :summary) "External edit")
          (emacsvox-aural--write-user-data changed)
          (should-error (emacsvox-aural-profile-set-startup-palette 'reading-owned))
          (should (equal changed (emacsvox-aural-read-user-data))))))))

(ert-deftest emacsvox-aural-voice-palettes-tools-portable-and-full-backup-differ ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-tuned-storage
    (let* ((before (emacsvox-aural-voice-data--entries 'reading emacsvox-aural-voice-palette-registry))
           (portable (emacsvox-aural-voice-data--export-effective emacsvox-aural-voice-palette-registry 'reading))
           (backup (emacsvox-aural-voice-data--backup emacsvox-aural-voice-palette-registry 'reading emacsvox-aural-routing--choice-sets))
           (inputs (emacsvox-aural-voice-data--read-exchange backup (emacsvox-aural-voice-palette 'acss-default)))
           (copy (emacsvox-aural-voice-data--materialize (plist-get inputs :registry) 'reading 'reading "Restored"
                                                       (plist-get inputs :choice-sets) '((bolden . "restored-bolden")))))
      (should (equal (plist-get portable :omitted-local-choices) '(bolden)))
      (should (eq (plist-get (plist-get portable :palette) :parent) 'acss-default))
      (should-not (string-match-p "reading-bolden-after" (prin1-to-string (plist-get portable :palette))))
      (should (= (length (plist-get backup :palettes)) 3))
      (should (= (length (plist-get backup :choice-sets)) 1))
      (should (equal (plist-get (car (plist-get copy :choice-sets)) :choices) (emacsvox-test--tuned-choices)))
      (should (equal (plist-get (car (plist-get copy :choice-sets)) :id) "restored-bolden"))
      (should (equal before (emacsvox-aural-voice-data--entries 'reading emacsvox-aural-voice-palette-registry)))
      (emacsvox-aural-voice-data--read-exchange (plist-get portable :palette) (emacsvox-aural-voice-palette 'acss-default))
      (setf (plist-get backup :choice-sets) nil)
      (should-error (emacsvox-aural-voice-data--read-exchange backup (emacsvox-aural-voice-palette 'acss-default))))))

(ert-deftest emacsvox-aural-voice-palettes-tools-reject-incomplete-and-dependent-exchanges ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-tuned-storage
    (should-error (emacsvox-aural-voice-data--read-exchange
                   (emacsvox-test--choice-fixture :expected-palette) (emacsvox-aural-voice-palette 'acss-default)))
    (let ((backup (emacsvox-aural-voice-data--backup emacsvox-aural-voice-palette-registry 'reading emacsvox-aural-routing--choice-sets)))
      (setq backup (plist-put backup :palettes (cl-remove 'acss-default (plist-get backup :palettes) :key (lambda (data) (plist-get data :id)))))
      (should-error (emacsvox-aural-voice-data--read-exchange backup (emacsvox-aural-voice-palette 'acss-default)))))
  (let ((export (emacsvox-aural-voice-data--export-effective emacsvox-aural-voice-palette-registry 'acss-default)))
    (should (eq (plist-get (plist-get export :palette) :id) 'acss-default-copy))
    (emacsvox-aural-voice-data--read-exchange (plist-get export :palette) (emacsvox-aural-voice-palette 'acss-default))))

(ert-deftest emacsvox-aural-voice-palettes-tools-export-failure-preserves-file ()
  :tags '(voice-palette-tools)
  (let ((file (make-temp-file "voice-export-")))
    (unwind-protect
        (progn
          (with-temp-file file (insert "Original"))
          (cl-letf (((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                    ((symbol-function 'rename-file) (lambda (&rest _) (error "Rename failure"))))
            (should-error (emacsvox-aural-voice-palettes--write-exchange '(:fixture t) file)))
          (should (equal "Original" (with-temp-buffer (insert-file-contents file) (buffer-string)))))
      (delete-file file))))

(ert-deftest emacsvox-aural-voice-palettes-tools-import-retries-frozen-complete-data ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-voice-save
    (let* ((file (expand-file-name "backup.el" directory))
           (emacsvox-aural-voice-palettes--import-pending nil)
           (before (emacsvox-aural-read-user-data))
           (current emacsvox-aural-voice-palette-override)
           (backup (emacsvox-aural-voice-data--backup emacsvox-aural-voice-palette-registry 'reading-owned emacsvox-aural-routing--choice-sets)))
      (emacsvox-aural-voice-palettes--write-exchange backup file)
      (cl-letf (((symbol-function 'read-file-name) (lambda (&rest _) file))
                ((symbol-function 'read-string) (lambda (&rest _) "restored"))
                ((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                ((symbol-function 'emacsvox-aural-voice-palettes--import-gaps) (lambda (&rest _) nil))
                ((symbol-function 'emacsvox-aural-ui-speak) #'ignore))
        (cl-letf (((symbol-function 'emacsvox-aural--write-user-data) (lambda (&rest _) (error "Second-store failure"))))
          (should-error (emacsvox-aural-voice-palettes-import) :type 'user-error))
        (should (equal before (emacsvox-aural-read-user-data)))
        (should (eq current emacsvox-aural-voice-palette-override))
        (let ((sets (plist-get (emacsvox-aural-read-routing-profiles) :choice-sets)))
          (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "Retry")))
            (should (eq (emacsvox-aural-voice-palettes-import) 'restored)))
          (should (equal sets (plist-get (emacsvox-aural-read-routing-profiles) :choice-sets)))))
      (should-not emacsvox-aural-voice-palettes--import-pending)
      (should (eq current emacsvox-aural-voice-palette-override))
      (should (equal (plist-get (emacsvox-aural-voice-runtime--resolve 'bolden 'restored) :choices)
                     (plist-get (emacsvox-aural-voice-runtime--resolve 'bolden 'reading-owned) :choices))))))

(ert-deftest emacsvox-aural-voice-palettes-tools-actions-are-reachable ()
  :tags '(voice-palette-tools)
  (with-temp-buffer
    (emacsvox-aural-voice-palettes-mode)
    (dolist (pair '(("U" . emacsvox-aural-voice-palettes-use-at-startup)
                    ("w" . emacsvox-aural-voice-palettes-export) ("W" . emacsvox-aural-voice-palettes-backup)
                    ("i" . emacsvox-aural-voice-palettes-import) ("u" . emacsvox-aural-voice-palettes-where-used)))
      (should (eq (key-binding (kbd (car pair))) (cdr pair)))))
  (with-temp-buffer
    (emacsvox-aural-voice-palette-previews-mode)
    (should (eq (key-binding (kbd "O")) 'emacsvox-aural-voice-palette-previews-open-source))
    (should (eq (key-binding (kbd "u")) 'emacsvox-aural-voice-palettes-where-used))))

(ert-deftest emacsvox-aural-voice-palettes-tools-edit-reviews-shared-descendants ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-voice-editor
    (emacsvox-aural-register-voice-palette-data
     '(:schema-version 3 :id inherited-test :summary "Shared child" :parent reading-owned :routing owned :entries nil))
    (emacsvox-aural-save-user-data)
    (let ((before (emacsvox-aural-read-user-data)) prompt)
      ...
      (cl-letf (((symbol-function 'yes-or-no-p) (lambda (text) (setq prompt text) nil)))
        (should-error (emacsvox-aural-voice-editor-save-to-collection) :type 'user-error))
      (should (string-match-p "inherited-test" prompt))
      (should (equal before (emacsvox-aural-read-user-data)))
      (should (emacsvox-aural-voice-drafts--dirty-fields (emacsvox-aural-voice-editor--draft))))))

(ert-deftest emacsvox-aural-voice-palettes-tools-import-refuses-new-name-conflict ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-voice-save
    (let* ((file (expand-file-name "portable.el" directory))
           (emacsvox-aural-voice-palettes--import-pending nil)
           (before (emacsvox-aural-read-user-data))
           (export (emacsvox-aural-voice-data--export-effective emacsvox-aural-voice-palette-registry 'reading-owned)))
      (emacsvox-aural-voice-palettes--write-exchange (plist-get export :palette) file)
      (cl-letf (((symbol-function 'read-file-name) (lambda (&rest _) file))
                ((symbol-function 'read-string) (lambda (&rest _) "newly-taken"))
                ((symbol-function 'emacsvox-aural-voice-palettes--import-gaps) (lambda (&rest _) nil))
                ((symbol-function 'yes-or-no-p)
                 (lambda (&rest _)
                   (emacsvox-aural-register-voice-palette-data
                    '(:schema-version 3 :id newly-taken :summary "Another action" :parent acss-default :routing owned :entries nil))
                   t)))
        (should-error (emacsvox-aural-voice-palettes-import) :type 'user-error))
      (should-not emacsvox-aural-voice-palettes--import-pending)
      (should (equal before (emacsvox-aural-read-user-data)))
      (should (equal (emacsvox-aural-voice-palette-summary (emacsvox-aural-voice-palette 'newly-taken)) "Another action")))))

(ert-deftest emacsvox-aural-voice-palettes-tools-reports-absent-inventory-choices ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-tuned-storage
    (cl-letf (((symbol-function 'tts-voice-inventory)
               (lambda () '(:engines ((:engine-id "dectalk" :availability "available" :default-voice-id "Paul"
                                      :voices ((:voice-id "Paul" :availability "available"))))))))
      (let ((gaps (emacsvox-aural-voice-palettes--import-gaps
                   (emacsvox-test--choice-fixture :expected-palette) emacsvox-aural-routing--choice-sets)))
        (should (= (length gaps) 2))
        (should (cl-some (lambda (gap) (string-match-p "bolden.*[Ee]loquence" gap)) gaps))
        (should (cl-some (lambda (gap) (string-match-p "smoothen" gap)) gaps))))))

(ert-deftest emacsvox-aural-voice-palettes-tools-open-source-preserves-session-selection ()
  :tags '(voice-palette-tools)
  (emacsvox-test--with-owned-runtime
    (let ((selected emacsvox-aural-voice-palette-override) buffer)
      (unwind-protect
          (save-window-excursion
            (cl-letf (((symbol-function 'tts-speak) #'ignore))
              (setq buffer (emacsvox-aural-list-voice-palette-previews 'source-child))
              (with-current-buffer buffer
                (emacsvox-aural-voice-palette-previews-refresh 'bolden)
                (call-interactively (key-binding (kbd "O")))
                (should (eq emacsvox-aural-voice-palette-previews-palette 'source-base))
