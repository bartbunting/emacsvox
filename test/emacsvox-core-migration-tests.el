...
(ert-deftest emacsvox-core-word-and-character-movement-use-navigation ()
  "Real movement commands label their output without changing direct reading."
  (with-temp-buffer
    (insert (propertize "Voice mapping" 'emacsvox-aural-facts
                        '(:role aural-interface :visibility folded)
                        'emacsvox-aural-module 'aural-tools))
    (let ((emacsvox-aural-submission-occasion nil)
          (emacsvox-aural-submission-context nil)
          (tts-stop-immediately nil)
          events)
      (cl-labels ((capture (text)
                   (let ((context (emacsvox-aural-capture-context)))
                     ...
                     (push (list (substring-no-properties text)
                                 (plist-get context :occasion)) events))))
        (cl-letf (((symbol-function 'tts-speak) #'capture)
                  ((symbol-function 'tts-letter) #'capture)
                  ((symbol-function 'emacsvox-icon) #'ignore))
          (dolist (case '((backward-word 6 "Voice") (left-word 6 "Voice")
                          (forward-word 1 "mapping") (right-word 1 "mapping")
                          (forward-char 1 "o") (right-char 1 "o")
                          (backward-char 2 "V") (left-char 2 "V")))
            (ert-info ((format "%s" (car case)))
              (goto-char (cadr case))
              (setq events nil)
              (funcall-interactively (car case) 1)
              (should (equal events (list (list (caddr case) 'navigation))))
              (should-not emacsvox-aural-submission-occasion)))
          ;; Helpers still support ordinary reading and explicit caller intent.
          (goto-char 1)
          (setq events nil)
          (emacsvox-speak-word)
          (should (equal events '(("Voice" continuous))))
          (let ((emacsvox-aural-submission-occasion 'inspection))
            (emacsvox-speak-word)
            (should (equal (car events) '("Voice" inspection))))
          ;; Internal movement used by reading must remain silent.
          (setq events nil)
          (forward-word 1)
          (backward-word 1)
          (should-not events))))))

(ert-deftest emacsvox-core-structural-movement-and-scrolling-use-navigation ()
  "Movement feedback is navigation; separate progress notices keep their lane."
  (save-window-excursion
    (with-temp-buffer
      (switch-to-buffer (current-buffer))
      (insert "One sentence.  Another sentence.\n\nSecond paragraph.\n\f\nLast page.\n")
      (dotimes (_ 80) (insert "More text for scrolling.\n"))
      (let ((emacsvox-aural-submission-occasion nil)
            (emacsvox-aural-submission-context nil)
        ...
        (cl-letf (((symbol-function 'tts-speak)
                   (lambda (text)
                     (push (list text (plist-get (emacsvox-aural-capture-context) :occasion)) events)))
                  ((symbol-function 'emacsvox-icon) #'ignore)
                  ((symbol-function 'tts-notify)
                   (lambda (&rest _) (push emacsvox-aural-submission-occasion notices))))
          (dolist (case '((forward-sentence 1) (backward-sentence 30)
                          (forward-paragraph 1) (backward-paragraph 50)
                          (forward-page 1) (backward-page 65)
                          (forward-sexp 1) (backward-sexp 4)
                          (scroll-up 1) (scroll-up-command 1)
                          (scroll-down 1000) (scroll-down-command 1000)))
            (ert-info ((format "%s" (car case)))
              (goto-char (cadr case))
              (set-window-start (selected-window) (if (> (point) 900) 700 1))
              (setq events nil notices nil)
              (funcall-interactively (car case) 1)
              (should events)
              (should (cl-every (lambda (event) (eq (cadr event) 'navigation)) events))
              (should (cl-every #'null notices))
              (should-not emacsvox-aural-submission-occasion)))
          (erase-buffer)
          (insert "Before ")
          (insert-text-button "Button" 'action #'ignore)
          (insert " After")
          (dolist (case '((forward-button 1) (backward-button 20)))
            (goto-char (min (cadr case) (point-max)))
            (setq events nil)
            (funcall-interactively (car case) 1)
            (should (equal events '(("Button" navigation)))))
          (goto-char 1)
          (setq events nil)
          (emacsvox-speak-region (point-min) (point-max))
          (should (eq (cadar events) 'continuous)))))))
