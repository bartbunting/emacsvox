...
(ert-deftest emacsvox-aural-tools-home-preserves-target-and-help-return ()
  ...
  (emacsvox-test--with-home-context
    ...
    (with-current-buffer "*Emacsvox Aural*"
      (should (eq (tabulated-list-get-id) 'explain))
...
(ert-deftest emacsvox-aural-home-search-retains-direct-feedback-actions ()
  "The short menu keeps old action names searchable with a visible return row."
  (emacsvox-test--with-home-context
    (emacsvox-aural-home-toggle-group)
    (should (equal (cl-loop for (id . _) in tabulated-list-entries
                           when (symbolp id) collect id)
                   '(explain recent-feedback return-source)))
    (dolist (choice '(("Change this feedback" . change-feedback)
                      ("Choose a voice for this item" . remap)
                      ("Edit the named voice used here" . tune-voice)
                      ("Change a sound for this item" . remap-earcon)
                      ("Semantic vocabulary" . semantics)
                      ("Explain this item" . explain)))
      (let (opened)
        (setq emacsvox-aural-home-expanded-groups nil)
        (emacsvox-aural-home-refresh)
        (cl-letf (((symbol-function 'completing-read)
                   (lambda (_prompt choices &rest _)
                     (should (assoc (car choice) choices))
                     (car choice)))
                  ((symbol-function 'emacsvox-aural-home--activate)
                   (lambda (id) (setq opened id))))
          (emacsvox-aural-home-search))
        (should (eq opened (cdr choice)))
        (should (eq (tabulated-list-get-id) 'explain))
        (should (memq 'understand emacsvox-aural-home-expanded-groups))))))
          ...
          (save-window-excursion
            ...
            (cl-letf
              ...
              (with-current-buffer "*Emacsvox Aural*"
                ...
                (should (string-prefix-p "Feedback for this item: " spoken))
                (emacsvox-aural-home-next)
                (should (string-prefix-p "Recent feedback: " spoken))
                (emacsvox-aural-home-next)
                (should (string-prefix-p "Return to source item: " spoken))
