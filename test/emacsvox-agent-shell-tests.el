...
(ert-deftest emacsvox-agent-shell-yank-dwim-speaks-interactive-text-once ()
  "Agent Shell text paste should inherit yank speech and prefix handling."
  (require 'emacsvox-advice)
  (emacsvox-agent-shell--install-advice)
  (with-temp-buffer
    (let ((kill-ring '("first paste" "second paste"))
          (kill-ring-yank-pointer nil)
          (interprogram-paste-function nil)
          events)
      (cl-letf (((symbol-function 'window-system) (lambda (&optional _) nil))
                ((symbol-function 'emacsvox-icon)
                 (lambda (icon) (push (list 'icon icon) events)))
                ((symbol-function 'emacsvox-speak-region)
                 (lambda (start end &rest _)
                   (push (list 'speak (buffer-substring start end)) events))))
        (call-interactively #'agent-shell-yank-dwim)
        (should (equal (buffer-string) "first paste"))
        (should (equal (nreverse events)
                       '((icon yank-object) (speak "first paste"))))
        (setq events nil)
        (let ((current-prefix-arg 2))
          (call-interactively #'agent-shell-yank-dwim))
        (should (equal (nreverse events)
                       '((icon yank-object) (speak "second paste"))))
        (setq events nil)
        (agent-shell-yank-dwim)
        (should-not events)))))

(ert-deftest emacsvox-agent-shell-yank-dwim-image-does-not-speak-old-region ()
  "An image paste should not be mistaken for a text yank."
  (require 'emacsvox-advice)
  (emacsvox-agent-shell--install-advice)
  (with-temp-buffer
    (insert "old text")
    ...
    (cl-letf (((symbol-function 'window-system) (lambda (&optional _) 'x))
              ...
              ((symbol-function 'agent-shell--save-clipboard-image)
               (lambda (&rest _) "/unused/image.png"))
              ((symbol-function 'agent-shell--get-files-context)
               (lambda (&rest _) "image context"))
              ((symbol-function 'agent-shell--shell-buffer) #'current-buffer)
              ((symbol-function 'agent-shell-insert)
               (lambda (&rest args) (insert (plist-get args :text))))
              ((symbol-function 'emacsvox-speak-region)
               (lambda (&rest _) (ert-fail "Image paste spoke an old region"))))
      (call-interactively #'agent-shell-yank-dwim)
      (should (equal (buffer-string) "old textimage context")))))
