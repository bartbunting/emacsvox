...
(defun omnivox--native-explanation (response expected)
  "Validate RESPONSE against frozen EXPECTED choice or playback evidence."
  (omnivox-parameters--require (<= (string-bytes (json-serialize response)) (* 64 1024)) "explanation size")
  (omnivox--choice-object-keys response '(:protocol_version :request_id :type :result))
  (omnivox-parameters--require
   (and (eql (plist-get response :protocol_version) 1)
        (equal (plist-get response :type) "voice_parameters_explained_v1")) "explanation response")
    ...
    (pcase (plist-get result :status)
      ...
      ("unavailable"
       ...
       (omnivox-parameters--require (member (plist-get result :reason)
                                          '("plan_expired" "native_unavailable" "voice_unavailable")) "explanation reason")
       (omnivox-parameters--text (plist-get result :message) 1024))
      ("ready"
       (omnivox--choice-object-keys result '(:status :evidence :plan_id :choice_id :realized :identity :parameters))
       (omnivox-parameters--identity (plist-get result :identity))
       (omnivox--choice-object-keys (plist-get result :realized) '(:engine_id :voice_id))
       (omnivox-parameters--id (plist-get (plist-get result :realized) :engine_id))
       (omnivox-parameters--text (plist-get (plist-get result :realized) :voice_id) 4096)
       (omnivox-parameters--require (equal (plist-get result :choice_id) (plist-get expected :choice-id)) "explanation choice")
       (let ((application (plist-get expected :application))
             (selector (plist-get expected :selector)))
         (if application
             (omnivox-parameters--require
              (and (equal (plist-get result :evidence) "adapter_applied")
                   (equal (plist-get result :plan_id) (plist-get application :plan_id))
                   (cl-every (lambda (key) (equal (plist-get (plist-get result :identity) key)
                                                  (plist-get (plist-get application :identity) key)))
                             '(:schema_id :profile_id :catalogue_revision :runtime_generation))
                   (cl-every (lambda (key) (equal (plist-get (plist-get result :realized) key)
                                                  (plist-get (plist-get expected :realized) key)))
                             '(:engine_id :voice_id))) "historical explanation owner")
           (omnivox-parameters--require
            (and (equal (plist-get result :evidence) "planned") (eq (plist-get result :plan_id) :null)
                 (equal (plist-get (plist-get result :realized) :engine_id) (plist-get selector :engine-id))
                 (or (not (eq (plist-get selector :kind) 'exact))
                     (equal (plist-get (plist-get result :realized) :voice_id) (plist-get selector :voice-id)))
                 (or (not (plist-get expected :native))
                     (equal (plist-get (plist-get result :identity) :schema_id)
                            (plist-get (plist-get expected :native) :schema-id)))) "planned explanation owner")))
       (omnivox-parameters--array (plist-get result :parameters) 64)
       (let (seen)
         (mapc
          (lambda (parameter)
            (omnivox--choice-object-keys parameter '(:id :value :origin :masked_native :read_back))
            (let ((id (plist-get parameter :id)) (value (plist-get parameter :value))
                  (origin (plist-get parameter :origin)))
              (omnivox-parameters--id id)
              (omnivox-parameters--require (not (member id seen)) "duplicate explanation parameter")
              (push id seen)
              (unless (eq value :null)
                (omnivox-parameters--require
                 (or (memq value '(t :false)) (omnivox-parameters--integer-p value)
                     (and (floatp value) (omnivox-parameters--finite-p value))
                     (and (stringp value) (progn (omnivox-parameters--id value) t))) "explanation value"))
              (omnivox-parameters--require (member origin '("engine_default" "common_mapping" "context_mapping" "native_set" "native_default")) "parameter origin")
              (dolist (key '(:masked_native :read_back)) (omnivox-parameters--boolean (plist-get parameter key)))
              (when (eq (plist-get parameter :read_back) t)
                (omnivox-parameters--require (and (equal (plist-get result :evidence) "adapter_applied")
                                                 (not (eq value :null))) "readback evidence"))
              (when (eq (plist-get parameter :masked_native) t)
                (omnivox-parameters--require (equal origin "context_mapping") "masked origin"))))
          (plist-get result :parameters)))
       (when-let* ((application (plist-get expected :application)))
         (omnivox-parameters--require
          (equal (sort (append (plist-get application :masked_parameters) nil) #'string-lessp)
                 (sort (cl-loop for parameter across (plist-get result :parameters)
                                when (eq (plist-get parameter :masked_native) t)
                                collect (plist-get parameter :id)) #'string-lessp)) "historical masking")))
      (_ (error "Unknown explanation status")))
    result))
