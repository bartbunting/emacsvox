...
(cl-loop for f in emacsvox-rpm-spec-insertion-commands
         do
         (eval
...
(cl-loop for f in emacsvox-rpm-spec-navigation-commands
         do
         (eval
          `(defadvice ,f (after emacsvox pre act comp)
             "speak."
...
(cl-loop for  f in emacsvox-rpm-spec-build-commands
         do
         (eval
          `(defadvice ,f (after emacsvox pre act comp)
             "Speak."
...
(cl-loop for f in emacsvox-rpm-spec-toggle-commands
         do
         (eval
          `(defadvice ,f (after emacsvox pre act comp)
             "Speak."
                 ...
                 (setq switch
                       (intern
                        (replace-regexp-in-string "toggle"
                                                  "spec"
                                                  toggle)))
                 (emacsvox-icon (if (eval switch) 'on 'off)))))))
