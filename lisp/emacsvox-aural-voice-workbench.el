...
(defvar-local emacsvox-aural-voice-workbench--language-order nil
  "Visible parent and tree order indexed by row ID for column sorting.")
...
(defun emacsvox-aural-voice-workbench--engine-order-description ()
  "Return concise saved or temporary engine-order status."
  (if emacsvox-aural-session-engine-order
      (format "%s first for session"
              (car emacsvox-aural-session-engine-order))
    "saved engine order"))
...
(defun emacsvox-aural-voice-workbench--header ()
  ...
  (if (emacsvox-aural-voice-workbench--library-p)
    ...
    (format
     ...
     (emacsvox-aural-voice-workbench--engine-order-description)
...
(defun emacsvox-aural-voice-workbench-status ()
  ...
  (let* ((inventory (tts-voice-inventory))
    ...
    (format
     "%s; %s%s; %d engine%s, %d voice%s; routing %s; %s"
...
(defun emacsvox-aural-voice-workbench--resolved-voice (logical-voice)
  "Resolve LOGICAL-VOICE with complete saved choices and staged engine policy."
...
(defun emacsvox-aural-voice-workbench--selectors (logical-voice)
  "Return the complete ordered choices for LOGICAL-VOICE."
  (plist-get (emacsvox-aural-voice-workbench--resolved-voice logical-voice) :selectors))

(defun emacsvox-aural-voice-workbench--logical-voices ()
  "Return effective named voices from the active palette."
  (sort (mapcar (lambda (entry) (symbol-name (car entry)))
                (emacsvox-aural-effective-voice-entries
                 (emacsvox-aural-voice-workbench--active-palette))) #'string-lessp))
...
(defun emacsvox-aural-voice-workbench--scope-description (_logical-voice)
  "Describe named choices separately from shared workstation policy."
  "palette-owned; policy shared")

(defun emacsvox-aural-voice-workbench--provenance-description (logical-voice)
  "Describe the saved source of LOGICAL-VOICE."
  (let ((voice (emacsvox-aural-voice-workbench--resolved-voice logical-voice)))
    (format "%s in %s" (or (plist-get voice :choice-source) 'automatic)
            (plist-get voice :palette))))

(defun emacsvox-aural-voice-workbench--palette-entry (logical-voice)
  "Return the complete model's named definition for LOGICAL-VOICE."
  (let* ((palette (emacsvox-aural-voice-workbench--active-palette))
         (resolved (emacsvox-aural-voice-runtime--resolve logical-voice palette
                                                        emacsvox-aural-voice-workbench-staged-profile)))
    (when (plist-get resolved :name)
      (cons (plist-get resolved :name) (plist-get resolved :definition)))))

(defun emacsvox-aural-voice-workbench--palette-aliases (logical-voice)
  "Return the canonical palette name for LOGICAL-VOICE."
  (when-let* ((name (plist-get (emacsvox-aural-voice-workbench--resolved-voice logical-voice) :name)))
...
(defun emacsvox-aural-voice-workbench--logical-row (logical-voice)
  ...
  (let* ((binding
          (emacsvox-aural-voice-workbench--resolved-voice logical-voice))
         (route
...
(defun emacsvox-aural-voice-workbench--sort-language-rows (a b column)
  ...
  (let* ((a-id (car a)) (b-id (car b))
         ...
         (a-order (gethash a-id emacsvox-aural-voice-workbench--language-order))
         (b-order (gethash b-id emacsvox-aural-voice-workbench--language-order))
         (reverse (cdr tabulated-list-sort-key)))
    (if (and (not a-group) (not b-group) (car a-order)
             (equal (car a-order) (car b-order)))
        (let ((a-value (downcase (aref (cadr a) column)))
      ...
      ;; Reverse column sorting must not reverse the tree or split a subtree.
      (< (cdr (if reverse b-order a-order))
         (cdr (if reverse a-order b-order))))))

(defun emacsvox-aural-voice-workbench--language-path (tag)
  "Return successive language and region prefixes from normalized TAG.
Private-use subtags identify voices, not extra branches such as X or GBCWMD."
  (let (prefix result)
    (catch 'private
      (dolist (part (split-string tag "-" t))
        (when (equal part "x") (throw 'private nil))
        (setq prefix (if prefix (concat prefix "-" part) part))
        (push prefix result)))
    (or (nreverse result) '(""))))

(defun emacsvox-aural-voice-workbench--group-languages (rows selected)
  "Group full physical ROWS in a language tree, revealing SELECTED's ancestors.
Single voices are direct rows.  Omit redundant branches with just one child."
  ...
  (setq emacsvox-aural-voice-workbench--voice-languages (make-hash-table :test #'equal)
        emacsvox-aural-voice-workbench--language-order (make-hash-table :test #'equal))
  (let ((groups (make-hash-table :test #'equal))
        (children (make-hash-table :test #'equal))
        (leaves (make-hash-table :test #'equal))
        (order 0) selected-tag result)
    (dolist (row rows)
      (let* ((tag (downcase (string-replace "_" "-" (aref (cadr row) 2))))
        ...
        (dolist (prefix (emacsvox-aural-voice-workbench--language-path tag))
          (unless (gethash prefix groups)
            (puthash parent (cons prefix (gethash parent children)) children))
          (puthash prefix (cons visible (gethash prefix groups)) groups)
          (setq parent prefix))
        (puthash parent (cons visible (gethash parent leaves)) leaves)))
    (setq selected-tag
          (if (emacsvox-aural-voice-workbench--language-row-p selected) (cadr selected)
            (gethash selected emacsvox-aural-voice-workbench--voice-languages)))
    (cl-labels
        ((add-row (row depth parent)
           (aset (cadr row) 0 (concat (make-string (* 2 depth) ?\s) (aref (cadr row) 0)))
           (puthash (car row) (cons parent (cl-incf order)) emacsvox-aural-voice-workbench--language-order)
           (push row result))
         (branches (tag)
           (sort (copy-sequence (gethash tag children))
                 (lambda (a b)
         ...
         (emit (tag depth automatic parent)
           (let ((members (gethash tag groups)) (nested (branches tag)) (direct (gethash tag leaves)))
             (cond
              ((= 1 (length members)) (add-row (car members) depth parent))
              ((and (null direct) (= 1 (length nested))) (emit (car nested) depth automatic parent))
              (t
               (let* ((key (emacsvox-aural-voice-workbench--language-key tag))
                      (state (gethash key emacsvox-aural-voice-workbench--language-expansion
                                      (if automatic 'expanded 'collapsed)))
                      (label (emacsvox-aural-voice-workbench--language-label tag))
                      (values (make-vector (length tabulated-list-format) "")))
                 (when (and selected-tag
                            (or (and (not (emacsvox-aural-voice-workbench--language-row-p selected))
                                     (equal tag selected-tag))
                                (string-prefix-p (concat tag "-") selected-tag)))
                   (setq state 'expanded))
                 (puthash key state emacsvox-aural-voice-workbench--language-expansion)
                 (when (> depth 0)
                   (setq label (car (last (split-string label ", " t)))))
                 (aset values 0 (emacsvox-aural-ui--expansion-text label (eq state 'expanded)))
                 (aset values 1 (format "%d voices; %s" (length members) state))
                 (add-row (list (list :language tag) values) depth parent)
                 (when (eq state 'expanded)
                   ;; Put all direct voice siblings together, including branches
                   ;; reduced to one voice, so any column sorts them consistently.
                   (dolist (child nested)
                     (when (= 1 (length (gethash child groups)))
                       (setq direct (append direct (gethash child groups)))))
                   (dolist (row (sort (copy-sequence direct)
                                     (lambda (a b) (string-lessp (downcase (aref (cadr a) 0))
                                                                (downcase (aref (cadr b) 0))))))
                     (add-row row (1+ depth) tag))
                   (dolist (child nested)
                     (when (> (length (gethash child groups)) 1)
                       (emit child (1+ depth) nil tag))))))))))
      (let ((roots (branches nil)))
        (dolist (tag roots) (emit tag 0 (= 1 (length roots)) nil))))
    (nreverse result)))
...
(defun emacsvox-aural-voice-workbench--active-profile ()
  "Return active routing profile data, or signal a user error."
  (if-let* ((entry
             (emacsvox-aural-routing-profile
              emacsvox-aural-active-routing-profile)))
      (emacsvox-aural-routing-profile-entry-data entry)
    (user-error "No active voice routing profile")))

(defun emacsvox-aural-voice-workbench--dirty-buffer ()
  "Return a live Voice Workbench buffer containing staged edits."
  (cl-find-if
   (lambda (buffer)
     (with-current-buffer buffer
...
(defun emacsvox-aural-voice-workbench--ensure-clean ()
  "Reject a quick routing change while Workbench edits are staged."
  (when-let* ((buffer (emacsvox-aural-voice-workbench--dirty-buffer)))
    (user-error
     "Save or cancel staged Voice Workbench edits in %s first"
     (buffer-name buffer))))

(defun emacsvox-aural-voice-workbench--usable-engine-p
    (engine disabled-engines)
  "Return non-nil when ENGINE can be preferred under DISABLED-ENGINES."
  (let ((engine-id (plist-get engine :engine-id))
        (health (format "%s" (or (plist-get engine :health) "unknown"))))
    (and engine-id
         (equal (plist-get engine :availability) "available")
         (not (member health '("failed" "unavailable")))
         (not (member engine-id disabled-engines)))))

(defun emacsvox-aural-voice-workbench--engine-candidates
    (&optional inventory profile)
  ...
  (let* ((inventory (or inventory (tts-voice-inventory)))
         ...
         (disabled (plist-get profile :disabled-engines)))
    (cl-loop
     for engine in (plist-get inventory :engines)
     when (emacsvox-aural-voice-workbench--usable-engine-p engine disabled)
     collect
     (let* ((engine-id (plist-get engine :engine-id))
            (position (cl-position engine-id order :test #'equal)))
       (cons
        (format "%s [%s; %s; %s]"
                (or (plist-get engine :display-name) engine-id)
                engine-id
                (or (plist-get engine :health) "unknown")
                (if position
                    (format "position %d" (1+ position))
                  "not ranked"))
        engine-id)))))

(defun emacsvox-aural-voice-workbench--read-engine ()
  "Read one currently usable speech engine ID."
  (let ((candidates
         (emacsvox-aural-voice-workbench--engine-candidates)))
    (unless candidates
      (user-error "No available speech engine can be preferred"))
    (cdr
     (assoc-string
      (completing-read
       "Prefer speech engine: " candidates nil 'must-match)
      candidates))))

(defun emacsvox-aural-voice-workbench--engine (engine-id inventory)
  "Return ENGINE-ID from normalized INVENTORY."
  (cl-find
   engine-id (plist-get inventory :engines)
   :key (lambda (engine) (plist-get engine :engine-id))
   :test #'equal))

(defun emacsvox-aural-voice-workbench--validate-engine
    (engine-id inventory profile)
  "Return ENGINE-ID when INVENTORY and PROFILE permit preferring it."
  (let ((engine
         (emacsvox-aural-voice-workbench--engine engine-id inventory)))
    (unless engine
      (user-error "Speech engine %s is not in the current inventory" engine-id))
    (when (member engine-id (plist-get profile :disabled-engines))
      (user-error "Speech engine %s is disabled by the active profile"
                  engine-id))
    (unless (equal (plist-get engine :availability) "available")
      (user-error "Speech engine %s is currently unavailable" engine-id))
    (when (member (format "%s" (plist-get engine :health))
                  '("failed" "unavailable"))
      (user-error "Speech engine %s is currently %s"
                  engine-id (plist-get engine :health)))
    engine-id))

(defun emacsvox-aural-voice-workbench--engine-display-name
    (engine-id inventory)
  "Return readable ENGINE-ID name from INVENTORY."
  (let ((engine
         (emacsvox-aural-voice-workbench--engine engine-id inventory)))
    (or (plist-get engine :display-name) engine-id)))

(defun emacsvox-aural-voice-workbench--preference-callback
    (display-name scope)
  "Return terminal apply callback for DISPLAY-NAME and preference SCOPE."
  (lambda (status)
    (pcase (plist-get status :status)
      ...
      ('partial
       (emacsvox-aural-voice-workbench--announce
        "%s preference applied to only some speech processes"
        display-name))
      (_
       (emacsvox-aural-voice-workbench--announce
        "Could not apply %s as preferred engine: %s"
        display-name (or (plist-get status :status) "unknown status"))))))

(defun emacsvox-aural-prefer-engine (engine-id &optional save)
  "Prefer ENGINE-ID globally while preserving explicit voice routes.
  ...
  (let* ((active-entry
    ...
    (if (not save)
      ...
      (let ((previous-session
            ...
            (preferred
             (emacsvox-aural-routing-prefer-engine-in-data
              profile engine-id)))
        (setq emacsvox-aural-session-engine-order nil)
        (condition-case error-data
            (emacsvox-aural-commit-routing-profile-data
             preferred emacsvox-aural-routing-profiles-file
             (emacsvox-aural-voice-workbench--preference-callback
              display-name 'saved))
          (error
           (setq emacsvox-aural-session-engine-order previous-session)
...
(defun emacsvox-aural-voice-workbench-move-preferred-engine-up ()
  "Move the selected engine earlier in preferred engine order."
  (interactive)
  (emacsvox-aural-voice-workbench--move-policy-engine 'preferred -1 "preferred order"))

(defun emacsvox-aural-voice-workbench-move-preferred-engine-down ()
  "Move the selected engine later in preferred engine order."
  (interactive)
  (emacsvox-aural-voice-workbench--move-policy-engine 'preferred 1 "preferred order"))
...
(defun emacsvox-aural-voice-workbench--profile-diagnostics (profile)
  ...
  (let (diagnostics checked-engines)
    ...
    (cl-labels
      ...
      (dolist (voice (emacsvox-aural-voice-workbench--logical-voices))
        (dolist (selector (emacsvox-aural-voice-workbench--selectors voice))
          (check-selector selector voice)))
...
(defun emacsvox-aural-voice-workbench--logical-preview-entry (logical-voice)
  "Return the saved named voice preview with staged workstation policy."
  (let ((voice (emacsvox-aural-voice-workbench--resolved-voice logical-voice)))
    (list :text emacsvox-aural-voice-workbench-preview-text
          :language (plist-get voice :language)
          :acss (emacsvox-aural-voice-workbench--preview-acss logical-voice)
          :rate-offset (emacsvox-aural-voice-workbench--preview-rate-offset logical-voice)
          :effects (emacsvox-aural-voice-workbench--preview-effects logical-voice)
          :selectors (copy-tree (plist-get voice :selectors))
          :fallback-policy (emacsvox-aural-voice-runtime--preview-policy voice)
          :disabled-engine-ids (copy-sequence (plist-get (plist-get voice :policy) :disabled-engines)))))
...
(defun emacsvox-aural-voice-workbench-refresh (&optional id)
  ...
  (let* ((selected (or id (gethash emacsvox-aural-voice-workbench-view
         ...
         (single (and (eq emacsvox-aural-voice-workbench-view 'physical)
                      ...
                      (cl-every (lambda (row)
                                  (or (emacsvox-aural-voice-workbench--language-row-p (car row))
                                      (car (gethash (car row) emacsvox-aural-voice-workbench--language-order))))
                                entries)
  ...
  (cond
   ((buffer-live-p emacsvox-aural-voice-workbench--details-parent)
    (let ((warning (emacsvox-aural-voice-workbench--dismiss-warning))
          (parent emacsvox-aural-voice-workbench--details-parent))
      (quit-window)
      (emacsvox-aural-ui-pop-to-buffer parent)
  ...
  (emacsvox-aural-ui-with-help-window
    (princ
     (concat
      ...
      "Save the named voice in that editor, with or without applying it.\n\n"
      "Engine policy is shared across palettes. In the engine view:\n"
      "O toggles preferred engines; [ and ] reorder them.\n"
      "f toggles fallback engines; { and } reorder them.\n"
      "D disables or enables an engine; K requests a recovery probe.\n"
      "w or C-c C-c saves and applies engine policy; u undoes a staged edit.\n"
      "C-c C-k cancels staged policy edits; r retries a saved apply.\n"
      "U restores the previous saved policy; E exports and I imports it.\n\n"
      "C-c C-a offers available actions; C-c C-i opens the offline manual.\n"
      "h opens Home; q leaves the Workbench, retaining staged policy edits.\n")))
...
(defun emacsvox-aural-voice-workbench--dismiss-warning ()
  ...
  (when (emacsvox-aural-voice-workbench--dirty-p)
    (concat
     "Voice Workbench hidden. Engine policy changes remain staged, "
  ...
  (let ((row (tabulated-list-get-id))
    ...
    (pcase command
      ...
      ((or 'emacsvox-aural-voice-workbench-move-preferred-engine-up
           ...
           'emacsvox-aural-voice-workbench-toggle-preferred-engine
