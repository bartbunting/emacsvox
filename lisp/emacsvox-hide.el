;;; emacsvox-hide.el --- Hide/Expose Text blocks   -*- lexical-binding: t; -*-

;; Copyright (C) 1995 -- 2024, T. V. Raman
;; Copyright (C) 2026 Emacsvox contributors
;; All Rights Reserved.
;; SPDX-License-Identifier: GPL-2.0-or-later

;; Author: T. V. Raman <tv.raman.tv@gmail.com>
;; Maintainer: Emacsvox contributors
;; Keywords: Emacsvox, Speak, Spoken Output, hide
;; URL: https://github.com/bartbunting/emacsvox

;; This file is part of Emacsvox.
;;
;; Emacsvox is free software; you can redistribute it and/or modify
;; it under the terms of the GNU General Public License as published by
;; the Free Software Foundation; either version 2, or (at your option)
;; any later version.
;;
;; Emacsvox is distributed in the hope that it will be useful,
;; but WITHOUT ANY WARRANTY; without even the implied warranty of
;; MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
;; GNU General Public License for more details.
;;
;; You should have received a copy of the GNU General Public License
;; along with Emacsvox.  If not, see <https://www.gnu.org/licenses/>.

;;; Commentary:
;; Flexible hide and show for emacsvox.
;; This module allows one to easily hide or expose
;; blocks of lines starting with a common prefix.
;; It is motivated by the need to flexibly hide quoted text in email
;; but is designed to be more general.
;; the prefix parsing is inspired by filladapt.el
;;; Code:

;;   Required modules:

(eval-when-compile (require 'cl-lib))
(require 'emacsvox-preamble)

;;;  voice locking for block header lines

(defvar emacsvox-hidden-header-line-personality voice-annotate
  "Personality used to identify header lines of blocks.")

;; forcibly set this to t to avoid a possible Emacs bug:
(cl-declaim (special line-move-ignore-invisible))
(setq line-move-ignore-invisible t)

;;;   Identifying the prefix

;;;   define parsing tables

(defvar emacsvox-hide-prefix-token-table
  '(
    ;; Included text in news or mail replies
    ("^[ \t]*[|>]+" citation)
    ;; Included text generated by SUPERCITE.  We can't hope to match all
    ;; the possible variations, your mileage may vary.
    ("^[ \t]*[A-Za-z0-9][^'`\"< \t\n]*>[ \t]*" supercite)
    ;; Lisp comments
    ("^[ \t]*;+" comment)
    ;; C comment block
    ("^[ \t]*/\\*+" comment)
    ("^[ \t]*\\*+" comment)
    ;; C++ comment block
    ("^[ \t]*//"  comment)
    ;; UNIX shell comments
    ("^[ \t]*#+" comment)
    ;; LaTeX comments
    ("^[ \t]*%+" comment)
    ;; Texinfo comments
    ("^[ \t]*@c[ \t]" comment)
    ("^[ \t]*@comment[ \t]" comment)
    )
  "Table of tokens emacsvox-hide knows about.
Format is

   ((REGEXP SYM) ...)

emacsvox-hide uses this table to build a tokenized representation of
the beginning of the current line.  Each REGEXP is matched
against the beginning of the line until a match is found.
Matching is done case-sensitively.  The corresponding SYM is
added to the list, point is moved to (match-end 0) and the
process is repeated.  The process ends when there is no REGEXP in
the table that matches what is at point.")

(defvar emacsvox-hide-prefix-not-token-table
  '(
    "^[ ]*$"
    "^[ \t]*[a-zA-Z]+ "
    )
  "List of regexps that can never be a token.
Before trying the regular expressions in emacsvox-hide-prefix-token-table,
the regexps in this list are tried.  If any regexp in this list
matches what is at point then the token generator gives up and
doesn't try any of the regexps in emacsvox-hide-prefix-token-table.

Regexp matching is done case-sensitively.")

;;;   parse the prefix

(defun emacsvox-hide-parse-prefix ()
  "Parse prefix   token after   point and return a prefix spec.
The tokens regular expressions are specified in
emacsvox-hide-prefix-token-table.  The list returned is of this form
  (SYM COL STRING)

SYM is a token symbol as found in emacsvox-hide-prefix-token-table.
COL is the column at which the token ended.
STRING is the token's text."
  (let ((token-list nil)
        (case-fold-search nil)
        token-table not-token-table)
    (save-excursion
      (forward-line 0)
      (catch 'done
        (setq not-token-table emacsvox-hide-prefix-not-token-table)
        (while not-token-table
          (if (looking-at (car not-token-table))
              (throw 'done nil))
          (setq not-token-table (cdr not-token-table)))
        (setq token-table emacsvox-hide-prefix-token-table)
        (while  token-table
          (cond
           ((null (looking-at (car (car token-table))))
            (setq token-table (cdr token-table)))
           (t                           ;got a match
            (goto-char (match-end 0))
            (setq token-list
                  (list (nth 1 (car token-table))
                        (current-column)
                        (buffer-substring-no-properties
                         (match-beginning 0)
                         (match-end 0))))
            (throw 'done token-list))))
        token-list))))

;;;   test for a prefix match

;; Return t if this line matches the specified prefix spec

(defun emacsvox-hide-prefix-matches-this-line (prefix)
  (unless (eobp)
    (string-equal (nth 2 prefix)
                  (buffer-substring-no-properties
                   (point) (+ (point) (nth 1  prefix))))))

;;;   hiding a block

(defun emacsvox-hide-current-block (prefix)
  "Hide block starting on current line identified by  PREFIX.
Blocks are portions of the buffer having a common prefix.
Hiding results in only the first line of the block being visible.
Returns t if a block was found and hidden."
  (let ((begin (line-beginning-position))
        (start nil)
        (continue t)
        (count 1))
    (save-excursion
      (cond
       ((not prefix)
        (message "Not on a block")
        nil)
       ((= 1 (forward-line 1))
        (message "At bottom of buffer. ")
        nil)
       (t                               ;start looking for a
                                        ;block
        (setq start (point))
        (unless (emacsvox-hide-prefix-matches-this-line prefix)
          (setq continue nil))
        (while (and continue (not (eobp)))
          (beginning-of-line 2)
          (unless (emacsvox-hide-prefix-matches-this-line prefix)
            (setq continue nil))
          (cl-incf count))
        (cond
         ((> count 1)
          (with-silent-modifications
            (add-text-properties start (point)
                                 (list 'invisible t
                                       'cursor-intangible t
                                       'intangible t))
            (add-text-properties
             begin (point)
             (list 'emacsvox-hide-block-prefix (nth 2  prefix)
                   'emacsvox-hidden-block (cl-first prefix)
                   'personality emacsvox-hidden-header-line-personality)))
          (message "Hid %s  %s lines"
                   count (cl-first prefix))
          t)
         (t (message "Not on a block") nil)))))))

;;;   Exposing a block

;; Hiding marks the body of a block to be invisible,
;; and marks the entire block
;; by setting property emacsvox-hidden-block to name of block
;; Use this to efficiently identify and unhide the block.

(defun emacsvox-hide-expose-block ()
  "Exposes currently hidden block."
  (let ((start nil)
        (end nil)
        (block-name (get-text-property (point) 'emacsvox-hidden-block)))
    (save-excursion
      (forward-line 0)
      (cond
       (block-name
        (setq start (point))
        (setq end
              (next-single-property-change (point) 'emacsvox-hidden-block
                                           (current-buffer) (point-max)))
        (with-silent-modifications
          (put-text-property start end
                             'emacsvox-hidden-block nil)
          (put-text-property start end
                             'emacsvox-hide-block-prefix nil)
          (add-text-properties start end
                               (list 'invisible nil
                                     'intangible nil
                                     'cursor-intangible nil
                                     'personality nil)))
        (message "Exposed %s block containing %s lines"
                 block-name
                 (count-lines start end))
        end)
       (t (message "Not on a hidden block")
          nil)))))

;;;   Hiding and exposing  all blocks in a buffer
;;;###autoload
(defun emacsvox-hide-all-blocks-in-buffer ()
  "Hide all blocks."
  (ems-with-messages-silenced
   (let ((count 0)
         (prefix nil))
     (save-excursion
       (goto-char (point-min))
       (while (not (eobp))
         (setq prefix (emacsvox-hide-parse-prefix))
         (cond
          ((and prefix
                (emacsvox-hide-current-block prefix))
           (cl-incf count)
           (goto-char
            (next-single-property-change (point)
                                         'emacsvox-hidden-block
                                         (current-buffer)
                                         (point-max))))
          (t (forward-line 1)))))
     (tts-speak
      (format "Hid %s blocks" count)))))

(defun emacsvox-hide-expose-hidden-blocks-in-buffer ()
  "Expose any hidden blocks in current buffer."
  (ems-with-messages-silenced
   (let ((count 0)
         (block-end nil))
     (save-excursion
       (goto-char (point-min))
       (while (not (eobp))
         (setq block-end (emacsvox-hide-expose-block))
         (cond
          (block-end
           (goto-char block-end)
           (cl-incf count))
          (t (forward-line 1)))))
     (tts-speak (format "Exposed %s blocks" count)))))

;;;  User interface
;; helper to get prefix
(defun emacsvox-hide-get-block-prefix ()
  
  (let ((block-prefix nil))
    (or (emacsvox-hide-parse-prefix)
        (when (and (not (looking-at "^[ \t]*$"))
                   (y-or-n-p "Define a new block prefix? "))
          (setq block-prefix
                (read-from-minibuffer "Specify prefix: "))
          (push
           (list
            (format "^%s" block-prefix)
            'custom)
           emacsvox-hide-prefix-token-table)
          (setq block-prefix
                (list 'custom
                      (length block-prefix)
                      block-prefix))))))
;;;###autoload
(defun emacsvox-hide-or-expose-block (&optional prefix)
  "Hide or expose a block of text.
 Optional interactive prefix arg causes all blocks in current
buffer to be hidden or exposed."
  (interactive "P")
  (save-excursion
    (tts-stop 'all)
    (forward-line 0)
    (cond
     (prefix                            ;work on entire buffer
      (let ((block (next-single-property-change (point-min)
                                                'emacsvox-hidden-block
                                                (current-buffer)
                                                (point-max))))
        (cond
         ((and block
               (or (get-text-property (point-min)
                                      'emacsvox-hidden-block)
                   (get-text-property block 'emacsvox-hidden-block)))
          (emacsvox-hide-expose-hidden-blocks-in-buffer))
         (t (emacsvox-hide-all-blocks-in-buffer)))))
     ((get-text-property (point) 'emacsvox-hidden-block)
      (emacsvox-hide-expose-block))
     (t
      (let ((block-prefix
             (emacsvox-hide-get-block-prefix)))
        (when block-prefix
          (emacsvox-hide-current-block  block-prefix)))))))
;;;###autoload
(defun emacsvox-hide-or-expose-all-blocks ()
  "Hide or expose all blocks."
  (interactive)
  (emacsvox-hide-or-expose-block 'all))

;;;   speaking blocks sans prefix

(defun emacsvox-hide-speak-block-sans-prefix ()
  "Speaks current block after stripping its prefix. "
  (interactive)
  (unless (get-text-property (point) 'emacsvox-hidden-block)
    (emacsvox-hide-current-block
     (emacsvox-hide-get-block-prefix)))
  (let ((scratch-buffer (get-buffer-create " *tts-scratch-buffer* "))
        (block nil)
        (contents nil)
        (start nil)
        (end nil)
        (block-prefix (get-text-property (point) 'emacsvox-hide-block-prefix)))
    (save-excursion
      (forward-line 0)
      (cond
       (block-prefix
        (setq start (point))
        (setq end
              (next-single-property-change (point) 'emacsvox-hide-block-prefix
                                           (current-buffer)
                                           (point-max)))
        (setq contents (buffer-substring start end))
        (setq block (concat "^"
                            (regexp-quote block-prefix)))
        (set-buffer scratch-buffer)
        (setq buffer-undo-list  t)
        (with-silent-modifications
          (erase-buffer)
          (insert contents)
          (put-text-property (point-min)
                             (point-max)
                             'invisible nil)
          (goto-char (point-min))
          (while (re-search-forward block nil t)
            (replace-match " ")))
        (emacsvox-speak-region (point-min) (point-max)))
       (t (message "Not on a hidden block"))))))

(provide 'emacsvox-hide)

;;; emacsvox-hide.el ends here
