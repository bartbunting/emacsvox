...
(defun ems--reftex-select-previous-heading-after (&rest _)
  ...
  (when (ems-interactive-p)
    (emacspeak-speak-line) (emacspeak-icon 'section)))
...
(defun ems--reftex-toc-goto-line-after (&rest _)
  "Speech enable  by speaking toc entry."
  (when (ems-interactive-p)
    (emacspeak-icon 'large-movement) (recenter 0)
    (cond (outline-minor-mode (emacspeak-outline-speak-this-heading))
	  (t (emacspeak-speak-predefined-window 1)))))
...
(defun ems--reftex-toc-goto-line-and-hide-after (&rest _)
  ...
  (when (ems-interactive-p)
    ...
    (if outline-minor-mode (emacspeak-outline-speak-this-heading)
...
(defun ems--reftex-toc-view-line-after (&rest _)
  "Speech enable  by speaking toc entry."
  (when (ems-interactive-p)
    (emacspeak-icon 'large-movement) (other-window 1) (recenter 0)
    (other-window 1) (emacspeak-speak-predefined-window 2)))
...
(defun ems--reftex-toc-toggle-follow-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-toc-follow-mode 'on 'off))
...
(defun ems--reftex-toc-toggle-labels-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-toc-include-labels 'on 'off))
...
(defun ems--reftex-toc-toggle-file-boundary-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-toc-include-file-boundaries 'on 'off))
...
(defun ems--reftex-toc-toggle-context-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-toc-include-context 'on 'off))
...
(defun ems--reftex-index-toggle-follow-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-index-follow-mode 'on 'off))
    (message "Turned %s follow mode. "
...
(defun ems--reftex-index-toggle-labels-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-index-include-labels 'on 'off))
    (message "Turned %s labels. "
...
(defun ems--reftex-index-toggle-file-boundary-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-index-include-file-boundaries 'on 'off))
    (message "Turned %s file boundary markers. "
...
(defun ems--reftex-index-toggle-context-after (&rest _)
  "speak."
  (when (ems-interactive-p)
    (emacspeak-icon (if reftex-index-include-context 'on 'off))
    (message "Turned %s context markers. "
...
(defun ems--reftex-highlight-after (&rest _)
  ...
  (let ((beg (ad-get-arg 1)) (end (ad-get-arg 2)))
    ...
    (emacspeak-speak-line) (sit-for 2)))
