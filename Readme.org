...
3. Select and verify Emacs:
   If Emacs 31 is not the system default, copy =local.mk.example= to the
   ignored =local.mk= and set =EMACS= there.  Then verify the selected binary:
   #+BEGIN_SRC sh
   make check-emacs
   #+END_SRC

4. Configure and compile the files:
   #+BEGIN_SRC sh
   make all
   #+END_SRC

5. Install and test Omnivox:
   Omnivox is the recommended speech server on Linux, macOS, and WSL2.
   Follow the [[file:info/backends.texi][Speech Backends guide]] for your
   platform.  The supported launcher can run its standalone check after
   validating the selected Emacs and Omnivox executable:
   #+BEGIN_SRC sh
   ./bin/emacsvox --check
   #+END_SRC
   Continue only after hearing both the test tone and speech.  Direct eSpeak,
   SwiftMac, macOS Python, Eloquence, and DECtalk servers are compatibility
   alternatives, not the initial-user path.

6. Prove an isolated Emacsvox start:
   #+BEGIN_SRC sh
   ./bin/emacsvox
   #+END_SRC
   This source-checkout launcher defaults to Omnivox, honors the =EMACS=
   selection in =local.mk=, requires Emacs 31 or newer, uses absolute checkout
   paths, and deliberately loads neither site files nor personal Emacs
   configuration.  Run =./bin/emacsvox --diagnose= to print its selections
   without producing audio or starting interactive Emacs.  This clean session
   is the first-use proof, not a replacement for your normal customized Emacs.

7. Set up normal customized starts:
   Add the backend selection to the shell startup file used to launch Emacs:
   #+BEGIN_SRC sh
   export TTS_PROGRAM=omnivox
   #+END_SRC
   Then add the following line to the top of your Emacs init file:
   #+BEGIN_SRC elisp
   (load-file "<emacsvox-dir>/lisp/emacsvox-setup.el")
   #+END_SRC
   Replace =<emacsvox-dir>= with the directory where you unpacked the sources.
   In a changing source checkout, rebuild with =make all= and keep
