...
This builds the Windows executable, the separate 32-bit Eloquence capture
helper, and a native copy of the platform-neutral eSpeak voice data, then
stages them with the required MinGW runtime DLLs under =servers/omnivox-bin=.
The voice data is required for the Windows eSpeak fallback to initialize.  The
helper uses a user-installed 32-bit ECI runtime; no proprietary DLL is copied
or distributed.
...
are available.  When the staged Eloquence helper can load ECI, its =v1= through
=v8= voices also appear in inventory for per-span logical routing.  A missing
helper, DLL, or native voice degrades through the configured alternatives.
...
=OMNIVOX_ELOQUENCE_HELPER=, and =OMNIVOX_ECI_DLL= to the Windows process.  The
last two are optional overrides; the staged helper and normal Freedom
Scientific ECI path need no configuration.
