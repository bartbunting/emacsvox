...
Omnivox is Emacsvox's multi-engine speech host.  Its current Windows build can
route speech among WinRT, eSpeak, and optional Eloquence and DECtalk helpers.
Build and stage its content-addressed Windows runtime from this checkout with:
...
5. Install a Text-to-Speech Engine:
   - Open-source eSpeak on Linux:
     ...
     2. Compile the Emacsvox eSpeak server:
        ...
        make espeak
        #+END_SRC
   - ViaVoice Outloud / IBM TTS (Eloquence):
     - Install a separately licensed compatible IBM TTS/Eloquence runtime;
       Emacsvox does not distribute it.  The Voxin site publishes current
       availability and compatibility information.  Its Cerence and
       Vocalizer voices are not supported by the Outloud adapter.  See
       [[https://voxin.oralux.net/rss.xml][Voxin news]] and the
       [[https://voxin.oralux.net][Voxin site]].
   - Mac:
     - You can use the built-in Mac TTS engine. Emacsvox includes a speech server for this purpose.

6. Set Up the TTS Engine:
   After installing a TTS engine, select it in your shell configuration:
