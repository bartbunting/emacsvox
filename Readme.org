...
The current routing slice does not retry a different engine after a backend
fails during synthesis, and immediate =tts_say= and letter commands still use
the preferred legacy engine.  Bounded runtime-failure retry and framed
=emacsvox_tx= delivery remain planned.
