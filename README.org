...
If you use WSL2 and do not yet have Emacs 31, this is the path you want.  Do
not install a separate Emacs package first: the guided Emacsvox installer uses
an existing Emacs 31 or newer when available and otherwise downloads and builds
the pinned stable Emacs release for your user.
...
2. Under WSL2, inspect the short binary installation path and then run it:
   ...
   installer uses an existing Emacs 31 or newer when available.  Otherwise it
   builds the pinned stable Emacs for your user, downloads and verifies the
   matching Omnivox Windows release, records both paths without editing a shell
   ...
   On GNU/Linux or macOS, install Emacs 31 or newer and follow the
   ...
   release.  If Emacs 31 is not the system =emacs=, set =EMACS= in the ignored
   =local.mk=.  Then run:
   #+BEGIN_SRC sh
   make check-emacs
   make bytecode
