* The Emacsvox Audio Desktop.
:PROPERTIES:
:TEXINFO_NODE_NAME: Audio Desktop
:ALT_TITLE: Audio Desktop
:DESCRIPTION: Explore the objects and gestures of the audio desktop.
:END:

#+cindex: Audio Desktop

This chapter describes the Emacsvox audio desktop and gives tips and
...
In the conventional world of visual @@texinfo:@acronym{GUI,Graphical User Interface}@@-based computing, these tools and information objects
manifest themselves as a collection of icons organized in a
two-dimensional work-area  @@texinfo:@MDash{}@@ this organization is designed to
...
In the spirit of a truly speech-enabled application, Emacsvox does
...
language'' of visual interaction in mind.  Instead, Emacsvox enables
...
perceive the Emacsvox environment as an electronic desktop.  This
...
** Objects Making Up The Emacsvox Desktop
:PROPERTIES:
...
Emacsvox desktop.  Any information presented by Emacs is placed in a
...
information objects such as @@texinfo:@acronym{WWW}@@ pages, email messages, output from
...
:DESCRIPTION: Object Oriented Desktop
:END:
...
programming languages, markup source e.g. @@texinfo:@acronym{HTML,Hypertext Markup Language}@@ or LaTeX documents and so on.

** Emacsvox Specializes Aural Interaction
:PROPERTIES:
:TEXINFO_NODE_NAME: Context-Sensitive Interaction
:ALT_TITLE: Context-Sensitive Interaction
:DESCRIPTION: Context-Sensitive Interaction
:END:

#+cindex: Context-Sensitive Interaction

The content-specific user interaction described above is a very
powerful feature of Emacs, and this is where Emacsvox derives its
...
ground-work for developing Emacsvox as a versatile aural counterpart
to the product of years of software engineering that has been invested
by the Emacs community.  In short, Emacsvox would not exist in its
...
Emacsvox takes advantage of the content-specific knowledge available
...
(@@texinfo:@acronym{ACSS}@@) in conjunction with the Emacs @@texinfo:@acronym{EWW,Emacs Web Wowser}@@ and @@texinfo:@acronym{W3}@@ Web browsers.
...
Emacsvox also exploits content-specific knowledge to provide
...
provides by default; in other cases, Emacsvox implements the
...
@@texinfo:@dfn{Selective Display}@@ lets one easily hide and expose portions of a
buffer based on indentation  @@texinfo:@MDash{}@@ this feature can be used to
...
Emacsvox environment provides speech-enabled navigation of the
various buffers that are currently open on the Emacsvox desktop via
Emacs' built-in list-buffers feature.  Emacs' dired  @@texinfo:@MDash{}@@ directory
editor  @@texinfo:@MDash{}@@ for browsing the file system, along with the new
...
currently open on the Emacsvox desktop.  This is @@texinfo:@b{very powerful}@@
 @@texinfo:@MDash{}@@ where a @@texinfo:@acronym{GUI,Graphical User Interface}@@ user is
typically limited to quickly locating an object from a relatively
small collection  @@texinfo:@MDash{}@@ the size of the collection being a direct
function of available display real-estate  @@texinfo:@MDash{}@@ the Emacsvox user
...
between two and three weeks  @@texinfo:@MDash{}@@ over that time I typically
...
@@texinfo:@acronym{WWW}@@ pages.

Ubiquitous search in the eyes-free environment is critical  @@texinfo:@MDash{}@@ as
...
interaction, and the typical mouse-enabled user @@texinfo:@b{never}@@ uses the
find dialog to find a submit button  @@texinfo:@MDash{}@@ it is simply more
...
limited to either tabbing through all the controls on a @@texinfo:@acronym{WWW}@@
