...
** Install Dependencies 
   ...
   type 
: npm install  
in ~$EMACSVOX_DIR/js/node~ to install dependencies.
...
** Connect From Within Emacsvox

  - First time: M-x package-install nvm  to configure Emacs to find
    the right ~node~ executable.
  - M-x load-library emacsvox-maths
  - M-x emacsvox-maths-start --- start the server/client.
  - Invoke the /Maths Navigator/ AKA emacsvox-maths muggle using key
    _s-SPC_ --- here _s-_ means press the Windows key.
  - Read the docs and profit --- see info node /emacsvox-maths/ in
    the Emacsvox manual.
