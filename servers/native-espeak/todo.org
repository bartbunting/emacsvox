...
- The server fully implements the Emacsvox server interface
- Users can switch languages and the voice used for each language.
  In general, the syntax for arguments to tts-set-language is
...
* Changes inherited from Emacspeak 56
** Relative pitch and volume
In order to ensure auditory feedback functions correctly no matter what voice
is selected, the mappings of Emacsvox's auditory faces to eSpeak commands
...
When calling tts-set-language (either interactively using C-e d S or in your init file)
