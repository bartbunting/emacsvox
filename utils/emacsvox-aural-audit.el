...
(defun emacsvox-aural-audit--insert-overview ()
  ...
  (insert
   ...
   "heard output, =P= replays its full ordered presentation, =c= auditions "
...
(defun emacsvox-aural-audit--insert-migration ()
  ...
  (insert
   ...
   "broader semantic rules remain in the one object-level resolution.  Keep a "
   "cue-only or implicit-content path on the legacy adapter until it can be "
   "represented without dropping tones, indentation, overlays, notification "
   "routing, or other established behavior.  Do not replace cue symbols with "
   "file paths and do not queue unresolved semantic names.\n\n"
